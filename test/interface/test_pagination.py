...
from sa_filters.pagination import Pagination
...
class TestWrongPagination(TestPaginationFixtures):
    ...
    def test_wrong_page_number(self, session, page_number, page_size):
        ...
        with pytest.raises(InvalidPage) as err:
            apply_pagination(stmt, page_number, page_size)

        expected_error = "Page number should be positive: {}".format(page_number)
    ...
    def test_wrong_page_number_with_no_results(self, session, page_number, page_size):
        stmt = select(Bar)
    ...
    def test_wrong_page_size(self, session, page_number, page_size):
        ...
        expected_error = "Page size should not be negative: {}".format(page_size)
        assert error_value(err) == expected_error
...
class TestNoPaginationProvided(TestPaginationFixtures):
    ...
    def test_no_pagination_info_provided(self, session):
        ...
        paginated_stmt, pagination = apply_pagination(
            stmt, page_number, page_size, total_results
        )

        assert stmt == paginated_stmt
        assert (
            Pagination(page_number=1, page_size=8, num_pages=1, total_results=8)
            == pagination
        )

        result = session.scalars(paginated_stmt).all()
...
class TestNoPageNumberProvided(TestPaginationFixtures):
    ...
    def test_page_size_provided(self, session):
        ...
        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=1, page_size=2, num_pages=4, total_results=8)
            == pagination
        )
...
class TestApplyPagination(TestPaginationFixtures):
    ...
    def test_page_size_zero_and_no_page_number_provided(self, session):
        ...
        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=1, page_size=0, num_pages=0, total_results=8)
            == pagination
        )
    ...
    def test_page_number_and_page_size_provided(self, session):
        ...
        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=3, page_size=2, num_pages=4, total_results=8)
            == pagination
        )
    ...
    def test_get_individual_record(self, session):
        ...
        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=5, page_size=1, num_pages=8, total_results=8)
            == pagination
        )
    ...
    @pytest.mark.usefixtures("multiple_bars_inserted")
    def test_page_number_greater_than_number_of_pages(self, session, page_number):
        stmt = select(Bar)
        page_size = 2
    ...
    def test_last_complete_page(self, session):
        ...
        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=4, page_size=2, num_pages=4, total_results=8)
            == pagination
        )
    ...
    def test_last_incomplete_page(self, session):
        ...
        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=2, page_size=5, num_pages=2, total_results=8)
            == pagination
        )
...
class TestQueryWithNoResults:
    def test_page_size_and_page_number_provided(self, session):
        ...
        paginated_stmt, pagination = apply_pagination(stmt, page_number, page_size)

        assert stmt != paginated_stmt
        assert (
            Pagination(page_number=1, page_size=2, num_pages=0, total_results=0)
            == pagination
        )
...
class TestQueryObject(TestPaginationFixtures):
    ...
    def test_page_number_and_page_size_provided(self, session):
        ...
        paginated_query, pagination = apply_pagination(
            query, page_number, page_size, total_results
        )
