...
from sa_filters.exceptions import BadFilterFormat, BadSpec, FieldNotFound
from test.models import Bar, Corge, Foo, Qux


ARRAY_NOT_SUPPORTED = "ARRAY type and operators supported only by PostgreSQL"


STRING_DATE_TIME_NOT_SUPPORTED = (
    "TODO: String Time / DateTime values currently not working as filters by SQLite"
)
...
class TestFiltersNotApplied:
    ...
    def test_wrong_filters_format(self, session, filter_):
        ...
        expected_error = "Filter spec `{}` should be a dictionary.".format(filter_)
    ...
    def test_invalid_field_but_valid_model_attribute(self, session, attr_name):
        ...
        expected_error = "Model <class 'test.models.Bar'> has no column `{}`.".format(
            attr_name
        )
        assert expected_error == err.value.args[0]
...
class TestMultipleModels:
    ...
    def test_multiple_models(self, session):
        ...
        assert len(result) == 4
        bars, quxs = zip(*result, strict=True)
...
class TestAutoJoin:
    ...
    def test_do_not_auto_join(self, session):
        ...
        with pytest.raises(BadSpec) as exc:
            apply_filters(stmt, filters, do_auto_join=False)
...
class TestApplyEqualToFilter:
    ...
    @pytest.mark.usefixtures("multiple_bars_inserted")
    def test_multiple_filters_applied_to_a_single_model(self, session, filters):
        stmt = select(Bar)
...
class TestApplyNotEqualToFilter:
    ...
    @pytest.mark.parametrize("operator", ["!=", "ne"])
    @pytest.mark.usefixtures("multiple_bars_inserted")
    def test_multiple_filters_applied_to_a_single_model(self, session, operator):
        stmt = select(Bar)
        filters = [
...
class TestDateFields:
    @pytest.mark.parametrize(
        "value", [datetime.date(2016, 7, 14), datetime.date(2016, 7, 14).isoformat()]
    )
    @pytest.mark.usefixtures("multiple_quxs_inserted")
    def test_filter_date_equality(self, session, value):
        stmt = select(Qux)
        filters = [{"field": "created_at", "op": "==", "value": value}]
    ...
    @pytest.mark.parametrize(
        "value", [datetime.date(2016, 7, 13), datetime.date(2016, 7, 13).isoformat()]
    )
    @pytest.mark.usefixtures("multiple_quxs_inserted")
...
class TestDateTimeFields:
    ...
    def test_filter_datetime_equality(self, session, is_sqlite, value):
        ...
        stmt = select(Qux)
        filters = [{"field": "execution_time", "op": "==", "value": value}]

        filtered_stmt = apply_filters(stmt, filters)
        result = session.execute(filtered_stmt).scalars().all()

        assert len(result) == 1
        assert result[0].execution_time == datetime.datetime(2016, 7, 14, 3, 5, 9)
    ...
    def test_filter_multiple_datetimes(self, session, is_sqlite, value):
        ...
        assert len(result) == 2
        assert result[0].execution_time == datetime.datetime(2016, 7, 13, 2, 5, 9)
        assert result[1].execution_time == datetime.datetime(2016, 7, 14, 3, 5, 9)
...
class TestApplyBooleanFunctions:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("hello", "`or` value must be an iterable across the function arguments"),
        ],
    )
    @pytest.mark.usefixtures("multiple_bars_inserted")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("hello", "`and` value must be an iterable across the function arguments"),
        ],
    )
    @pytest.mark.usefixtures("multiple_bars_inserted")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("hello", "`not` value must be an iterable across the function arguments"),
        ],
    )
    @pytest.mark.usefixtures("multiple_bars_inserted")
        ...
        filters = [
            {
                "and": [
                    ...
                    {"not": [{"field": "name", "op": "==", "value": "name_2"}]},
                ],
            }
        ]
...
class TestHybridAttributes:
    ...
    def test_orm_descriptors_not_valid_hybrid_attributes(
        ...
        stmt = select(Bar)
        filters = [{"model": "Bar", "field": field, "op": "==", "value": 100}]
    ...
    def test_filter_by_hybrid_properties(self, session):
        stmt = select(Bar, Qux)
        filters = [
            {"model": "Bar", "field": "count_square", "op": "==", "value": 100},
            {"model": "Qux", "field": "count_square", "op": ">=", "value": 26},
        ]
    ...
    def test_filter_by_hybrid_methods(self, session):
        stmt = select(Bar, Qux)
        filters = [
            {"model": "Bar", "field": "three_times_count", "op": "==", "value": 30},
            {"model": "Qux", "field": "three_times_count", "op": ">=", "value": 31},
        ]
